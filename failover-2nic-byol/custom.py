...
def create_bigip_deployment(context, instance_number):
    ...
    for nics in range(2):
        ...
        if nics == 0:
            ...
            network_ip = context.properties[
                "bigIpExternalSelfIp0" + str(instance_number)
            ]
        else:
        ...
        storage_config = [
            {
                ...
                "labels": {"f5_cloud_failover_label": context.properties["cfeTag"]},
            }
        ]
    else:
        storage_config = []
    ...
    additionalMetadataTags.update(
        {"bigip-peer-addr": context.properties["bigIpPeerAddr"]}
    ) if instance_number == 2 else None

    # Populate Example VIPs
    public_ip_name = generate_name(prefix, "public-ip-01")
    depends_on_array.append(public_ip_name)
    additionalMetadataTags.update(
        {"service-address-01-public-ip": "$(ref." + public_ip_name + ".address)"}
    )
    ...
    bigip_config = [
        {
            ...
            "properties": {
                ...
                "targetInstances": [{"name": "bigip-vm-0" + str(instance_number)}],
                "uniqueString": context.properties["uniqueString"],
                "zone": context.properties["zone"],
            },
            "metadata": {"dependsOn": depends_on_array},
        }
    ]
    return bigip_config
    ...
    dag_configuration = [
        {
            ...
            "properties": {
                "firewalls": [
                    {
                        "allowed": [
                            {"IPProtocol": "TCP", "ports": [22, mgmt_port, 443]}
                        ],
                    ...
                    {
                        "allowed": [
                            {"IPProtocol": "TCP", "ports": [4353, 443]},
                            {"IPProtocol": "UDP", "ports": [1026]},
                        ],
                ...
                "forwardingRules": [
                    {
                        ...
                        "description": 'f5_cloud_failover_labels={"f5_cloud_failover_label":"'
                        + context.properties["cfeTag"]
                        + '","f5_target_instance_pair":"'
                        + target_instance_name
                        + ","
                        + target_instance_name2
                        + '"}',
                        "metadata": {"dependsOn": fr_depends_on_array},
                    }
                ],
                "healthChecks": [
                    {
...
def generate_config(context):
    ...
    resources = (
        ...
        + create_bigip_deployment(context, 1)
        + create_bigip_deployment(context, 2)
    ...
    outputs = outputs + [
        {"name": "deploymentName", "value": deployment_name},
        {"name": "bigIpInstanceId1", "value": "$(ref." + bigip_instance_name + ".id)"},
        {"name": "bigIpInstanceId2", "value": "$(ref." + bigip_instance_name2 + ".id)"},
        {"name": "bigIpInstanceName1", "value": bigip_instance_name},
        {"name": "bigIpInstanceName2", "value": bigip_instance_name2},
        {
        ...
        {"name": "bigIpUsername", "value": "admin"},
        {
            "name": "vip1PrivateIp1",
            "value": "$(ref."
            + bigip_instance_name
        ...
        {"name": "vip1PublicIp", "value": "$(ref." + fr_name + ".IPAddress)"},
        {
